...
export class SolanaService {
    ...
    public async getAccountForMnemonic(mnemonic: string): Promise<AccountInfo<Buffer>> {
        const seed = (await bip39.mnemonicToSeed(mnemonic)).subarray(0, 32);
        const faucetKeypair = Keypair.fromSeed(seed);
        const client = this.getClient();
        const account = await client.getAccountInfo(faucetKeypair.publicKey, 'confirmed');

        return account;
    }

    public async getAccountForAddress(address: string): Promise<AccountInfo<Buffer>> {
        const client = this.getClient();
        const account = await client.getAccountInfo(new PublicKey(address), 'confirmed');

        return account;
    }


    public async sendPayment(fromMnemonic: string, toAddress: string, amount: number): Promise<string> {
    ...
    public async ensureSufficientBalance(address: string, amount: number) {
        ...
        throw new BadRequestException(address, `Insufficient balance: ${account?.lamports || 0}`);
    }
