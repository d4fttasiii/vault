...
export const VaultConfig = (): Config => ({
  app: {
    port: process.env.VAULT_APP_PORT
      ? parseInt(process.env.VAULT_APP_PORT, 10)
      : 3000,
  },
  solana: {
    cluster:
      process.env.VAULT_SOLANA_CLUSTER || 'https://api.devnet.solana.com',
  },
  mongoDb: {
    endpoint:
      process.env.VAULT_MONGODB_ENDPOINT ||
  ...
  ipfs: {
    ...
    port: process.env.VAULT_S3_PORT
      ? parseInt(process.env.VAULT_S3_PORT, 10)
      : 9000,
