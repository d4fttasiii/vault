...
export class EncryptionService {
    ...
    encryptString(content: string, key: string): string {
        const encrypted = CryptoJS.AES.encrypt(content, key, { format: new JsonFormatter() });
        return encrypted.toString();
    }

    decryptString(cypher: string, key: string): string {
        return CryptoJS.AES.decrypt(cypher, key, { format: new JsonFormatter() }).toString(CryptoJS.enc.Utf8);
    }
}
